...
% 3. Play the original audio signal to ensure it is heard before any
%    processing is applied. Pause for the duration of the audio plus an
%    additional second to allow the playback to complete.
%
%
% 4. Iterate over different window sizes (N = 3, 11, 31):
...
%      window function.
%    - Play the filtered audio signal after the original audio signal with
%      a one-second delay between them to allow for comparison.
%
%
% 5. Compute and plot the time domain analysis:
%    - Plot the original and filtered audio signals in the time domain to
%      visualize any changes introduced by the filtering process.
%
% 6. Compute the FFT (Fast Fourier Transform) of the original and filtered
%    audio signals to obtain their magnitude spectra for frequency domain
%    analysis.
% 
% 7. Plot the frequency domain analysis:
%    - Plot the magnitude spectrum of the original and filtered signals in
%      the frequency domain to observe how the filtering affects different
%      frequency components..
%
% 8. Display the plots for each window size (N = 3, 11, 31) in separate
%    figures to facilitate comparison and analysis of the effects of
%    different window sizes on the audio signal filtering.
%
% Dependencies:
