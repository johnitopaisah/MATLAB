...
%     STUDENT_2 NAME: Blesso Danny Jeba Stephen - 63291
%
%
%   Introduction:
%    This MATLAB script demonstrates basic audio signal processing 
%    techniques, including Fast Fourier Transform (FFT), frequency 
%    manipulation, and inverse FFT.
%    It reads an audio signal from a WAV file, performs frequency 
%    manipulation, and plays both the original and processed audio signals.
%
%    Usage:
%    1. Prerequisites: MATLAB software installed on your system.
%    2. Download the Code: Download the MATLAB script (`SignalSwap.m`)
%       to your working directory.
%    3. Download Audio File: Ensure you have an audio file named 
%       `canal.wav` in the same directory as the MATLAB script.
%    4. Run the Script: Open MATLAB, navigate to the directory containing 
%       the script, and run the script by executing the command 
%       `SignalSwap.m` in the MATLAB command window.
%    5. Listening to the Audio: The script will play the original audio 
%       signal first, followed by the processed audio signal.
%       Listen to both signals to observe the difference.
%    6. Visualizing Frequency Content: The script will also generate plots 
%       of the FFT magnitude before and after processing,
%       allowing you to visualize the frequency content of the signals.
%
%    Code Explanation:
%    - The script begins by clearing the command window, closing all open 
%      figures, and reading the audio signal from the file 'canal.wav'.
%    - It then plays the original audio signal using the `sound` function 
%      and waits for the playback to finish.
%    - Next, the script calculates the FFT of the original signal and 
%      performs frequency manipulation to swap high and low frequencies.
%    - The processed signal is obtained by converting the manipulated FFT 
%      back to the time domain using the inverse FFT.
%    - The processed signal is played using the `sound` function, and plots
%      of the FFT magnitude before and after processing
%      are generated for visualization.
%
%    Files:
%    - `SignalSwap.m`: MATLAB script for audio signal processing.
...
%
%    Note:
%    - Ensure that the audio file 'canal.wav' is present in the same 
...
% Perform a Fast Fourier Transform (FFT) of the signal and store the 
% results in fftResult.
fftResult = fft(originalSignal);

% Calculate the magnitude of the FFT of the signal and store i
% t in fftMagnitude.
...
% Create a new complex vector newFftResult from the FFT results
...
% Calculate the magnitude of the newFftResult vector and store it in 
% newFftMagnitude
